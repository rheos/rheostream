...
| --- | --- | --- | --- |
| `core` | `apps/core` | HTTP API on the `api` surface, MCP on the `mcp` surface, identity endpoints under `/auth/*` on every application host, the internal API for the web tier | FastAPI under uvicorn. Holds every module's Python package. The MCP facade is built (`apps/mcp`) but not yet mounted in this process, so nothing answers on the `mcp` surface today. |
