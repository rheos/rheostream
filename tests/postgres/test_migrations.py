...
def test_no_alembic_ini_is_tracked_and_each_chain_knows_its_revisions() -> None:
    ...
    # the control chain ships two files and the core chain seven, so these are all
...
def test_core_chain_creates_exactly_the_twenty_four_tables(
    ...
    with engine.connect() as connection:
        # The version table holds one row on a linear chain: the head, not every
        # revision the code carries.
        assert recorded_revisions(connection, CORE_CHAIN) == {"0007_record_deletion"}
...
def test_failing_core_revision_marks_unavailable_and_startup_completes(
    ...
    # send repair's upgrade back through that revision's create_all against tables
    # that already exist.
    with broken_engine.begin() as connection:
        connection.execute(
            text("INSERT INTO core.alembic_version_core (version_num) VALUES (:v)"),
            {"v": "0007_record_deletion"},
        )
    repair(broken)
